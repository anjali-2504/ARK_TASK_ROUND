...
long long int  acc00,acc01,acc10,acc11;
int ib=4;
int ii;
    for (ii = 0; ii < sizen; ii += ib)
    {
        for (j = 0; j < sizen; j+=2)
        {
        ...
        {   acc00 = acc01 = acc10 = acc11 = 0;
            for (k = 0; k < sizen; k++)
            {
             acc00 +=MaxcostMat[k][j + 0] *  MincostMat[i + 0][k];
            acc01 +=MaxcostMat[k][j + 1] * MincostMat[i + 0][k];
            acc10 +=MaxcostMat[k][j + 0] * MincostMat[i + 1][k];
            acc11 += MaxcostMat[k][j + 1] * MincostMat[i + 1][k];
            }
             productMat[i + 0][j + 0] = acc00;
             productMat[i + 0][j + 1] = acc01;
             productMat[i + 1][j + 0] = acc10;
             productMat[i + 1][j + 1] = acc11;
        }
        }
    }
